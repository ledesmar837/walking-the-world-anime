        ...
        <div className="container-main py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10">
            ...
            <div>
              ...
              {/* Shop Anime — Top 5 Franquicias */}
              <div className="mt-10">
                <SectionHeading
                  title="Shop de Anime"
                  subtitle="Figuras LED, GK, articuladas y coleccionables"
                  icon="🛒"
                  href="/shop"
                />
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                  {[
                    { icon: '🐉', name: 'Dragon Ball', slug: 'dragon-ball', sub: 'Goku · Vegeta · Broly' },
                    { icon: '🏴‍☠️', name: 'One Piece', slug: 'one-piece', sub: 'Luffy · Zoro · Shanks' },
                    { icon: '🗡️', name: 'Solo Leveling', slug: 'solo-leveling', sub: 'Jinwoo · Beru · Igris' },
                    { icon: '🍥', name: 'Naruto', slug: 'naruto', sub: 'Sabio · Itachi · Madara' },
                    { icon: '⚔️', name: 'Demon Slayer', slug: 'demon-slayer', sub: 'Tanjiro · Rengoku · Nezuko' },
                  ].map((cat) => (
                    <Link
                      key={cat.slug}
                      href={`/shop/${cat.slug}`}
                      className="p-4 rounded-xl bg-[var(--color-surface-card)] border border-[var(--color-border)] hover:border-[var(--color-primary)]/40 hover:-translate-y-1 transition-all text-center group"
                    >
                      <span className="text-3xl block mb-2">{cat.icon}</span>
                      <span className="text-sm font-bold text-[var(--color-text-primary)] group-hover:text-[var(--color-primary)] transition-colors block">
                        {cat.name}
                      </span>
                      <span className="text-[0.55rem] text-[var(--color-text-tertiary)] mt-0.5 block">
                        {cat.sub}
                      </span>
                    </Link>
                  ))}
